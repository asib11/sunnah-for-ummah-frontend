...
export default function CategoryPageClient({ handle }: { handle: string }) {
  ...
  const displayProducts = productsData?.products?.map((p: any) => {
    ...
    return { name: p.title, price, originalPrice, image: p.thumbnail, variantId: lowestVariant?.id, handle: p.handle };
  }) ?? [];
