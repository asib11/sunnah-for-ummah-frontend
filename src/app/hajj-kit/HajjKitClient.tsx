...
import { CheckCircle2, Circle, Sparkles, ShoppingCart, ArrowRight, Package } from "lucide-react";
...
// Skeleton for kit items
const KitItemSkeleton = () => (
  <div className="flex items-center gap-4 p-5 rounded-[1.5rem] border-2 border-neutral-100 bg-white/60 animate-pulse">
...
export default function HajjKitClient() {
      ...
      <main className="flex-1 w-full max-w-6xl mx-auto px-4 py-12 md:py-20 mt-16 overflow-hidden">
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {Array.from({ length: 9 }).map((_, i) => <KitItemSkeleton key={i} />)}
          </div>
        ) : kitItems.length === 0 ? (
              ...
              {filteredItems.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className={`relative w-16 h-16 rounded-2xl overflow-hidden flex-shrink-0 bg-neutral-100 border transition-all ${selected ? "border-primary/30" : "border-neutral-100"}`}>
                      <img
                        src={item.thumbnail}
                        alt={item.en}
