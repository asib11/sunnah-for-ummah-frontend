...
import videoAsset from "../../public/baggy-sweatpants-black-cinematic.mp4.asset.json";
import sweatpantsImage from "@/assets/baggy-sweatpants-black.png";
...
const BaggySweatpants = () => {

  const router = useRouter();
  const { data, isLoading } = useQuery({
    queryKey: ["product", "baggy-sweatpants"],
    queryFn: () => storeApi.getProductByHandle("baggy-sweatpants"),
  });
  const { price } = getProductPrices(data);
  const [revealed, setRevealed] = useState(true);
  const { urls } = useSectionMedia("baggy-sweatpants", [
    { key: "video", label: "Background video", kind: "video", defaultUrl: "/baggy-sweatpants-black-cinematic.mp4" },
    ...
    >
      ...
      >
        <div className="relative h-full w-full max-w-2xl mx-auto">
          ...
          <img
            src={urls.image}
            alt="Baggy Sweatpants — washed black modest fit"
        ...
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div className="max-w-xl">
            <p className="font-body text-[9px] md:text-[10px] uppercase tracking-[0.3em] text-accent/90 mb-1">
              Sunnah Streetwear · 06
            </p>
            <h3 className="font-display text-xl md:text-3xl font-semibold text-primary-foreground leading-tight">
              Baggy <span className="italic text-accent">Sweatpants</span>
            </h3>
            <p className="font-body text-[10px] md:text-xs text-primary-foreground/65 mt-1.5 leading-relaxed">
              Modest, oversized cut with a relaxed drop — heavyweight loopback fleece, drawstring waist, and a generous straight leg that flows with you through every prayer and every street.
            </p>
          ...
          <button onClick={(e) => { e.stopPropagation(); router.push("/products/baggy-sweatpants"); }} className="group/btn inline-flex items-center gap-2 bg-accent text-accent-foreground pl-4 pr-1.5 py-1.5 rounded-full text-xs font-body font-semibold hover:bg-accent/90 transition-all shadow-lg shadow-accent/30 shrink-0">
            Shop Baggy
...
export default BaggySweatpants;
