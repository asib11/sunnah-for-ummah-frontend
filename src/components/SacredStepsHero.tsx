...
const SacredStepsHero = ({ onNavigateToKit }: Props = {}) => {
  ...
  return (
    <section className="relative w-full h-full min-h-[520px] overflow-hidden rounded-2xl">
      ...
      <div className="relative h-full flex flex-col items-center justify-center text-center px-6 md:px-10 py-12 [text-shadow:0_2px_24px_hsl(var(--foreground)/0.85)]">
        {/* Title */}
        <h1 className="font-display text-5xl md:text-6xl lg:text-7xl font-bold text-cream leading-[1.05] tracking-tight animate-fade-in drop-shadow-[0_4px_18px_hsl(var(--foreground)/0.7)]">
        ...
        {/* Description */}
        <p className="font-body text-sm md:text-base font-medium text-cream mt-6 leading-relaxed max-w-md animate-fade-in [text-shadow:0_1px_10px_hsl(var(--foreground)/0.85)]">
        ...
        <div className="mt-7 flex flex-wrap items-center justify-center gap-3 animate-fade-in">
          <button
            type="button"
            onClick={goToKit}
            className="group relative inline-flex items-center gap-3 rounded-full px-7 py-3 font-body text-sm font-semibold text-cream
              bg-primary-foreground/10 backdrop-blur-md border border-cream/25
        ...
        <div className="mt-8 flex items-center gap-4 md:gap-8">
          ...
          >
            ...
            <div className="font-body text-[10px] md:text-xs text-cream/75 uppercase tracking-wider mt-1.5 group-hover:text-cream transition-colors">
              Essential Items
            </div>
          </button>
          ...
          <div className="text-center">
            ...
            <div className="font-body text-[10px] md:text-xs text-cream/75 uppercase tracking-wider mt-1.5">
              Package Options
            </div>
          </div>
          <div className="w-px h-10 bg-primary-foreground/20" />
        ...
        <p
          key={`lbl-${active}`}
          className="mt-10 font-body text-[10px] md:text-xs uppercase tracking-[0.3em] text-cream/70 animate-fade-in"
        >
          {slides[active].label}
        </p>
        ...
        {/* Scroll cue */}
        <ChevronDown className="mt-8 w-5 h-5 text-cream/60 animate-bounce" />
      </div>
    </section>
  );
};
