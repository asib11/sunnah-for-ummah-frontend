...
import { Check, Star, CheckCircle2, Loader2, ShoppingCart, Upload, X, ChevronLeft, ChevronRight, Settings, ArrowLeftRight } from "lucide-react";
...
const HajjPackages = () => {
  ...
  useEffect(() => {
    try { window.localStorage.setItem(SETTINGS_KEY, JSON.stringify({ direction: slideDirection, intervalSec: slideIntervalSec })); } catch { /* ignore */ }
  ...
  const totalPages = Math.ceil(displayPackages.length / PER_PAGE);
  const visiblePackages = displayPackages.slice(page * PER_PAGE, page * PER_PAGE + PER_PAGE);
            ...
            <>
              <div className="mt-10 flex items-center justify-center gap-3">
                <button
                  ...
                  className="w-10 h-10 rounded-full bg-card border border-primary/20 flex items-center justify-center text-primary hover:bg-primary hover:text-primary-foreground transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                >
              ...
              <div className="mt-6 flex flex-col items-center gap-3">
                <button
                  ...
                  className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-card border border-primary/20 font-body text-xs font-semibold text-primary hover:bg-primary hover:text-primary-foreground transition-colors"
                >
                ...
                {showSettings && (
                  <div className="w-full max-w-md rounded-2xl bg-card/90 backdrop-blur border border-primary/15 shadow-md p-5 flex flex-col gap-4 animate-in fade-in zoom-in duration-300">
                    {/* Direction */}
